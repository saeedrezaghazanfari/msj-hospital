...
def national_code_val(national_code, ischeck_unique):
...
def name_val(name):
    ...
    if len(name) >= 30:
        raise forms.ValidationError(_('این فیلد باید کمتر از 30 کاراکتر باشد.'))
    for i in name:
        if i.isdigit():
...
def email_val(email, ischeck_unique):
